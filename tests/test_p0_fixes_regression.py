...
def create_mock_character(
    ...
    char.hidden_karma = hidden_karma
...
class TestBUG004InnerDemonKarma:
    """BUG-004: 测试心魔值和业力系统影响机制"""
    ...
    def test_karma_reduces_breakthrough_rate(self):
        """测试业力值降低突破率（每点业力降低2%）"""
        from backend.services.calc_service import get_breakthrough_rate
        
        char = create_mock_character(hidden_karma=0, hidden_inner_demon=0, hidden_luck=0)
        rate_without_karma = get_breakthrough_rate(char)
        
        char.hidden_karma = 10
        rate_with_karma = get_breakthrough_rate(char)
        
        # 业力值10应该降低突破率 10 * 0.02 = 0.2
        expected_reduction = 10 * 0.02
        actual_reduction = rate_without_karma - rate_with_karma
        assert abs(actual_reduction - expected_reduction) < 0.001, \
            f"业力值影响不正确: 预期降低{expected_reduction}，实际降低{actual_reduction}"
        
        print(f"✓ BUG-004: 业力值10降低突破率{expected_reduction:.2f}（2%每点）")
    
    def test_karma_reduces_explore_reward(self):
        """测试业力值降低探索收益（每点业力降低1%）"""
        from backend.services.calc_service import get_explore_reward_bonus
        
        char = create_mock_character(hidden_karma=0)
        bonus_without_karma = get_explore_reward_bonus(char)
        
        char.hidden_karma = 20
        bonus_with_karma = get_explore_reward_bonus(char)
        
        # 业力值20应该降低探索收益 20 * 0.01 = 0.2
        expected_reduction = 20 * 0.01
        actual_reduction = bonus_without_karma - bonus_with_karma
        assert abs(actual_reduction - expected_reduction) < 0.01, \
            f"业力值对探索收益影响不正确: 预期降低{expected_reduction}，实际降低{actual_reduction}"
        
        print(f"✓ BUG-004: 业力值20降低探索收益约{expected_reduction:.2f}（1%每点）")
...
def run_all_tests():
    ...
    # BUG-004 测试
    print("-" * 60)
    print("BUG-004: 心魔值和业力系统影响机制")
    print("-" * 60)
    try:
        test_bug004 = TestBUG004InnerDemonKarma()
        test_bug004.test_inner_demon_reduces_breakthrough_rate()
        test_bug004.test_karma_reduces_breakthrough_rate()
        test_bug004.test_karma_reduces_explore_reward()
